...
// Accepts GitHub's "issues" webhook, validates its signature, and drops a
// SyncMessage on the queue. Deliberately does no Asana work itself — that
// happens in ProcessSyncMessage, so a slow/failing Asana call never risks
// GitHub's webhook delivery timeout (10s) or retries.
//
...
// ignored action look identical from the outside, which avoids turning the
// endpoint into an oracle for guessing the webhook secret.
    ...
    ILogger<ReceiveGitHubWebhook> logger)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var signature = request.Headers.TryGetValues("X-Hub-Signature-256", out var values)
        ...
        {
            logger.LogInformation("Ignoring issue action '{Action}' — not one this project syncs.", payload.Action);
            return null;
        }

        var action = payload.Action.ToLowerInvariant() switch
        {
            ...
            _ => throw new InvalidOperationException($"Unhandled action '{payload.Action}'."),
        };
        ...
        logger.LogInformation(
            "Queuing sync for {Repository}#{IssueNumber} ({Action}).",
            message.Repository, message.IssueNumber, message.Action);
